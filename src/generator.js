...
function registerAdvancedRoute(app, pool, endpoint) {
  ...
  app.post(base, async (req, res) => {
    try {
      ...
      // 0) Column selection (new)
      const selectRaw = (req.query.select || '').trim();
      const allCols = tableMeta.columns.map(c => c.COLUMN_NAME);
      let selectList = '*';
      if (selectRaw) {
        const parts = selectRaw.split(',')
          .map(s => s.trim())
          .filter(s => allCols.includes(s))
          .map(s => `[${s}]`);
        if (parts.length) selectList = parts.join(', ');
      }
...
function registerRoutes(app, tableMeta, endpoint) {
  ...
  app.get(base, async (req, res) => {
    try {
      ...
      // 1) Column filters (with '~' escape for reserved names, comma-separated OR, ranges, and negation)
      const reserved = new Set(['order','limit','offset','select']);
      const where = [];
      for (const col of tableMeta.columns) {
        const keyDirect = col.COLUMN_NAME;
        const keyEscaped = `~${col.COLUMN_NAME}`;
        let v;
        if (Object.prototype.hasOwnProperty.call(req.query, keyEscaped)) {
          v = req.query[keyEscaped];
        } else if (!reserved.has(keyDirect) && Object.prototype.hasOwnProperty.call(req.query, keyDirect)) {
          v = req.query[keyDirect];
        }
        if (v !== undefined) {
          const rawParts = Array.isArray(v) ? v : [v];
          const tokens = rawParts
            .flatMap(x => String(x).split(','))
            .map(s => s.trim())
            .filter(s => s.length > 0);

          const typ = String(col.DATA_TYPE || '').toLowerCase();
          const posConds = []; // OR-ed within the same column
          const negConds = []; // AND-ed within the same column
          const eqVals = [];   // literals to combine into IN()
          let idx = 0;

          const addParam = (name, value) => {
            request.input(name, mapSqlTypeToMssqlType(col), value);
          };

          // Range tokens: () exclusive, [] inclusive (math convention)
          const isRangeToken = t => (/^[\(\[]/.test(t) && t.includes(':') && /[\)\]]$/.test(t));
          const parseRange = t => {
            const m = t.match(/^([\(\[])?\s*([^:]*)\s*:\s*([^\]\)]*)\s*([\)\]])?$/);
            if (!m) return null;
            const open = m[1] || '(';  // default exclusive
            const loRaw = m[2].trim();
            const hiRaw = m[3].trim();
            const close = m[4] || ')';
            // () exclusive, [] inclusive
            const loOp = open === '[' ? '>=' : '>';
            const hiOp = close === ']' ? '<=' : '<';
            return { loRaw, hiRaw, loOp, hiOp };
          };

          for (const t0 of tokens) {
            let t = t0;

            // Negation of a single value: !x (works for strings/numbers)
            if (t.startsWith('!') && t.length > 1 && !isRangeToken(t.slice(1))) {
              const pName = `${col.COLUMN_NAME.replace(/[^A-Za-z0-9_]/g, '_')}__ne_${idx++}`;
              negConds.push(`[${col.COLUMN_NAME}] <> @${pName}`);
              addParam(pName, t.slice(1));
              continue;
            }

            const tl = t.toLowerCase();
            if (tl === 'is.null') { posConds.push(`[${col.COLUMN_NAME}] IS NULL`); continue; }
            if (tl === 'not.null') { posConds.push(`[${col.COLUMN_NAME}] IS NOT NULL`); continue; }
            if ((tl === 'is.true' || tl === 'is.false') && (typ === 'bit' || typ.includes('int'))) {
              const val = (typ === 'bit') ? (tl === 'is.true') : (tl === 'is.true' ? 1 : 0);
              const pName = `${col.COLUMN_NAME.replace(/[^A-Za-z0-9_]/g, '_')}__b_${idx++}`;
              posConds.push(`[${col.COLUMN_NAME}] = @${pName}`);
              addParam(pName, val);
              continue;
            }
            ...
            if (isRangeToken(t)) {
              ...
              if (r) {
                const parts = [];
                if (r.loRaw !== '') {
                  const pLo = `${col.COLUMN_NAME.replace(/[^A-Za-z0-9_]/g, '_')}__lo_${idx++}`;
                  parts.push(`[${col.COLUMN_NAME}] ${r.loOp} @${pLo}`);
                  addParam(pLo, r.loRaw);
                }
                if (r.hiRaw !== '') {
                  const pHi = `${col.COLUMN_NAME.replace(/[^A-Za-z0-9_]/g, '_')}__hi_${idx++}`;
                  parts.push(`[${col.COLUMN_NAME}] ${r.hiOp} @${pHi}`);
                  addParam(pHi, r.hiRaw);
                }
                if (parts.length) posConds.push(`(${parts.join(' AND ')})`);
              }
              continue;
            }
          ...
          if (eqVals.length === 1) {
            const pName = `${col.COLUMN_NAME.replace(/[^A-Za-z0-9_]/g, '_')}__eq_0`;
            posConds.push(`[${col.COLUMN_NAME}] = @${pName}`);
            addParam(pName, eqVals[0]);
          } else if (eqVals.length > 1) {
            const names = eqVals.map((_, i) => `${col.COLUMN_NAME.replace(/[^A-Za-z0-9_]/g, '_')}__eq_${i}`);
            posConds.push(`[${col.COLUMN_NAME}] IN (${names.map(n => '@' + n).join(', ')})`);
            names.forEach((n, i) => addParam(n, eqVals[i]));
          }

          if (posConds.length || negConds.length) {
            if (posConds.length && negConds.length) {
              where.push(`((${posConds.join(' OR ')}) AND (${negConds.join(' AND ')}))`);
            } else if (posConds.length) {
              where.push(`(${posConds.join(' OR ')})`);
            } else {
              where.push(`(${negConds.join(' AND ')})`);
            }
          }
        }
      }

      // 2) Parse sort & pagination (Supabase/PostgREST) (Supabase/PostgREST) (Supabase/PostgREST) (Supabase/PostgREST) (Supabase/PostgREST) (Supabase/PostgREST)
      ...
      if (limit < 0 && offset === 0) {
        // no limit, no offset
        sqlText = `SELECT ${selectList} FROM ${qName(schema, table)} ${whereSql} ${orderSql}`;
      } else if (limit < 0 && offset >= 0) {
        // offset only
        request.input('offset', sql.Int, offset);
        sqlText = `SELECT ${selectList} FROM ${qName(schema, table)} ${whereSql} ${orderSql} OFFSET @offset ROWS`;
      } else {
        // limit >= 0
        request.input('offset', sql.Int, offset);
        request.input('limit', sql.Int, limit);
        sqlText = `SELECT ${selectList} FROM ${qName(schema, table)} ${whereSql} ${orderSql} OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`;
      }
